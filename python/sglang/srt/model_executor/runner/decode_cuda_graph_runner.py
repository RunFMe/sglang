...
class DecodeCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def execute(
        ...
        with timer_ctx, self.backend.replay_session():
            ...
            # Publish a read-done event for the WAR barrier once the forward is
            # done reading the shared req_to_token / SWA buffers. A cuda-graph
            # forward finishes those reads when the metadata snapshot is built
            # (here, pre-replay) and then replays reading only the static page
            # table, so the next schedule's WAR write can wait on this finer event
            # rather than the whole forward. Plain DECODE and DFLASH TARGET_VERIFY
            # both qualify (eagle publishes from draft_extend; ngram excluded).
